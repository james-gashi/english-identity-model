...
### BERT
BERT is a transformer model released in the late 2010s that is trained on token/sentence prediction, utilizing contextual embeddings. It is the defacto standard for NLP experiments, and proves to be a powerful and versatile model. We specifically used BERT base, which contains 110 million parameters. 
BERT has wrestled with us these past few weeks, refusing to run on CURC, and being finnicky in local environments. At 3 epochs, running on the Macbook Air M1 it actually performs worse than Distilibert, and needs around 45 minutes to run. Training loss and validation loss are both extremely high, both above 1.5, and accuracy sits around 40%, and grew minimally after epoch 2. The reason why this may be eludes us (could be our tokening?) but the trouble BERT base model has provided us proves that DistiliBERT is the superior option here.
### DistilBERT
**Description**
